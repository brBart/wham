        ...
        $(document).ready(function() {
            ...
            $("#sbmtBtn").bind("click", function(e) {
                ...
                if ($.trim($("#reason").val()).length == 0 ) {
                    alert("Please type in a valid reason for why you are suspending this reseller");
                    return;
                }
                
                cnf = confirm("Do you wish to suspend the reseller: " + $("#user").val());
                ...
                $.ajax({
                    type: "POST",
                    url: "<?=site_url() . "/servers/viewserver/whm_suspendreseller/" . $s_id . "/" ?>",
                    data: $("#suspendForm").serialize(),
                    dataType: "text",
                    success: function(text) {
                        $("#status_msg_div").addClass("well").html(text).show();
                        $("#results_div p").html("<p align=right><a class='btn btn-success' href='<?=site_url() . "/servers/viewserver/whm_suspendreseller/" . $s_id . "/" ?>'><span class='btn-label'>Go back</span></a><br/></p><b>RESULT:</b>");
                    }
                });
        ...
        <div class="row-fluid">
          <div class="span12">
            <div>
                ...
                <p><b>Suspend A Reseller</b></p><hr/>
     <div id="reseller_div">
     <?php if (isset($reseller_list)) :?>    
     <form id="suspendForm">
         <p>&nbsp;<br/>Reseller to suspend:<br/>
         ...
         <p>Reason For Suspension:<br/>
         <input type="text" id="reason" name="reason" placeholder="Reason for suspension..">
         </p>
         <p>Suspend all sub accounts for this user? (time consuming if reseller has many accounts)<br/>
         <input type="radio" name="suspend_all_sub_accounts" value="yes"> &nbsp; Yes<br/>
         <input type="radio" name="suspend_all_sub_accounts" value="no" checked> &nbsp; No</p>
         <p><button id="sbmtBtn" type="success" class="btn btn-success"><i class="icon-ban-circle icon-white"></i> Suspend Reseller</button></p>
     </form>
         <p>&nbsp;</p><p>&nbsp;</p>
