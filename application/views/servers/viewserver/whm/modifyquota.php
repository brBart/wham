            ...
            $("#modify_sbmt").bind("click", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(text) {
                        ...
                        $("#results_div p").html("<p align=right><a class='btn btn-success' href='<?=site_url() . "/servers/viewserver/whm_modifyquota/" . $s_id . "/" ?>'><span class='btn-label'>Go back</span></a><br/></p><b>RESULT:</b>");
                    }
                });    
            });
            <?php else :?>
        ...
        <div class="row-fluid">
          <div class="span12">
            <div>
                ...
                <p><b>Quota Modification</b> / <a href="<?= site_url() ?>/servers/viewserver/whm_modifyquota_multi/<?= $s_id ?>/">Modify Quota For Multiple Accounts</a></p><hr/>
     <div id="modify_div">
     <?php if (isset($account_selected)) :?>
         <form id="modify_acct_form">           
             <input type="hidden" name="user" value="<?= $account_selected->user ?>">
             Set new quota for user <b><font color="green">"<?= $account_selected->user ?>"</font></b> to:<br/>&nbsp;<br/>
             <input type="text" class="input-medium" id="quota" name="quota" value="<?= str_replace("M", "", $account_selected->disklimit) ?>"> MB
             <hr/></p>
                  <button class="btn btn-success" href="#" id="modify_sbmt">
                <span class="btn-label"><i class="icon-check icon-white"></i> Modify quota</span>
              </button>
         </form>
     <?php else :?>
         <p>Select the account to modify its quota</p>
         <form action="<?= site_url()?>/servers/viewserver/whm_modifyquota/<?= $s_id?>/" method="post">
