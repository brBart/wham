...
$(document).ready(function() {
    ...
    $(".removeParked").bind("click", function(e) {
        e.preventDefault();
        $("#parked_rm_result").children().remove();
        $("#parked_rm_result").hide();
        ...
        choice = confirm("You are about to delete the parked domain " + domain + ". \n\nDo you wish to continue?");
        ...
        $("#hiddenRemoveParkedForm input[name=domain]").val(domain);
        ...
        $.ajax({
            type: "POST",
            url: "<?= site_url() ?>/servers/cpanel/remove_parked/<?= $s_id ?>/<?= $account ?>/",
            data: $("#hiddenRemoveParkedForm").serialize() ,
            dataType: "json",
            success: function(text) {
                $("#parked_rm_result").html(text.msg);
                $("#parked_rm_result").show();
                $(cur).next("span").hide();
                if (text.status == 1) {
                    if ($(cur).parents("tbody").children().length == 1) {
                        $(cur).parents("tbody").append('<tr><td colspan="3">No results to display</td></tr>');
                    }
                    $(cur).parents("tr").remove();
                }
                setTimeout('$("#parked_rm_result").hide()', 3000);
                subRemoveLock = false;
            },

            error: function() {
                $("#parked_rm_result").html("Connection Error");
                $("#parked_rm_result").show();
                $(cur).next("span").hide();
                setTimeout('$("#parked_rm_result").hide()', 3000);
                subRemoveLock = false;
    ...
    $("#addNewDomainBtn").bind("click", function(e) {
        e.preventDefault();
        $("#parked_add_result").children().remove();
        $("#parked_add_result").hide();
        ...
        $.ajax({
            type: "POST",
            url: "<?= site_url() ?>/servers/cpanel/create_parked/<?= $s_id ?>/<?= $account ?>/",
            data: $("#newParkedForm").serialize() ,
            dataType: "json",
            success: function(text) {
                $("#parked_add_result").html(text.msg);
                $("#parked_add_result").show();
                if (text.status == 1) {
                    location.reload(true);
                } else {
                    $("#addNewDomainBtn").removeClass("disabled");
                    $("#loading").hide();
                    setTimeout('$("#parked_add_result").hide()', 10000);
                }
            },

            error: function() {
                $("#parked_add_result").html("Connection Error");
                $("#parked_add_result").show();
                $("#addNewDomainBtn").removeClass("disabled");
                $("#loading").hide();
                setTimeout('$("#parked_add_result").hide()', 3000);
            }
        });
    });
        ...
        <div class="row-fluid cpaneldivcontainer">
            ...
            <div>
                <ul class="nav nav-tabs">
                    ...
                    <li class="active"><a href="#"><b>Parked</b></a></li>
            ...
            <p>&nbsp;</p><p class="alert alert-success"><i class="icon-hand-right"></i> <b>Current Parked Domains</b></p>
                <table class="table table-bordered table-striped">
                    <thead><tr><th>Domain Name</th><th class="hidden-phone">Directory</th><th>Actions</th></tr></thead>
                    <tbody>
                    <?php if (isset($parkedlist["data"]) && is_array($parkedlist["data"]) && (!isset($parkedlist["data"]["domain"])) && count($parkedlist["data"]) > 0) :?>
                    <?php foreach ($parkedlist["data"] as $park ) :?>
                        <tr domain="<?= $park["domain"]?>">
                            <td><a href="http://<?= $park["domain"]?>/" target="_blank"><?= $park["domain"]?></a></td>
                            <td class="hidden-phone"><?= $park["dir"] ?></td>
                            <td><a href="#" class="removeParked" domain="<?= $park["domain"]?>">remove</a>
                            <span style="display:none">&nbsp; <img src='<?= base_url() . "includes/images/working.gif" ?>'></span></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php elseif (isset($parkedlist["data"]) && is_array($parkedlist["data"]) && (isset($parkedlist["data"]["domain"]))) :?>
                        <tr domain="<?= $parkedlist["data"]["domain"]?>">
                            <td><a href="http://<?= $parkedlist["data"]["domain"]?>/" target="_blank"><?= $parkedlist["data"]["domain"] ?></a></td>
                            <td class="hidden-phone"><?= $parkedlist["data"]["dir"] ?></td>
                            <td><a href="#" class="removeParked" domain="<?= $parkedlist["data"]["domain"] ?>">remove</a>
                            <span style="display:none">&nbsp; <img src='<?= base_url() . "includes/images/working.gif" ?>'></span></td>
                        </tr>
                    <?php else :?>
                        <tr><td colspan="3">No results to display</td></tr>
                    <?php endif; ?>
                    </tbody>    
                </table><p>&nbsp;</p><div id="parked_rm_result" class="well" style="display:none"></div><p>&nbsp;</p>
            <p>&nbsp;</p><p class="alert alert-success"><i class="icon-hand-right"></i> <b>Add A New Parked Domain</b></p>
            <form class="form-horizontal" id="newParkedForm">
                <div class="control-group">
                    <label class="control-label" for="domain">Parked domain name</label>
                    <div class="controls"><input type="text" id="parked_newdomain" name="domain" placeholder="domain name"></div>
                </div>
                <div class="control-group">
                    <div class="controls"><button type="submit" class="btn btn-success" id="addNewDomainBtn"><i class="icon-plus-sign icon-white"></i> Create Parked Domain</button> <span id="loading" style="display:none">&nbsp; <img src='<?= base_url() . "includes/images/working.gif" ?>'></span>
                    </div>
                </div>
            </form><p>&nbsp;</p><div id="parked_add_result" class="well" style="display:none"></div><p>&nbsp;</p>
            <p><i class="icon-hand-right"></i> <b>Note</b><br/>&nbsp;</p>
            <p><i>Parked domain name</i> : The domain name you wish to park.</p>
            <p>&nbsp;</p>
            <p>&nbsp;</p><p>&nbsp;</p>
        <form id="hiddenRemoveParkedForm">
