...
class Serversearch extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('servers/serversearch');
    }
    ...
    public function search() {
        ...
        if (!in_array($_POST["what"], array("server", "datacenter"))) {
            echo "<i>Invalid argument 1</i>";
            exit;
        }
        
        if (!in_array($_POST["for"], array("s_name", "s_hostname", "s_ip", "any", "dc_name", "dc_location"))) {
        ...
        if ($_POST["what"] == "server") {
            $this->db->from("server");
            $this->db->join("datacenter", 'datacenter.dc_id = server.s_dc', "left");
            $this->db->join("panels", "panels.p_id = server.s_cp", "left");
        }
        else {
            $this->db->select('count(server.s_dc) AS count'); 
            $this->db->from("datacenter");
            $this->db->join("server", 'server.s_dc = datacenter.dc_id', "left");
            $this->db->group_by("datacenter.dc_id");
            $this->db->order_by("datacenter.dc_name", "asc");
        }
        ...
        if ($_POST["for"] != "any") {
            ...
            if ($_POST["what"] == "server")
                $arr = array("s_name", "s_hostname", "s_ip");
            else
                $arr = array("dc_name", "dc_location");
        ...
        if ($_POST["what"] == "server")
            $this->load->view("servers/search/server", $args);
        else
            $this->load->view("servers/search/datacenter", $args);
